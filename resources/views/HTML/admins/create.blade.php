...
<i class="ti ti-chevron-right text-xs flex-shrink-0 text-default-400 rtl:rotate-180"></i>
<span class="font-medium text-default-700">Add Admin</span>
@endsection
...
<div class="card max-w-3xl">
    <div class="card-header border-b border-default-200 pb-4 mb-6 flex items-center gap-3">
        ...
        <div>
            <h5 class="text-base font-semibold text-default-800">New Admin</h5>
            <p class="text-xs text-default-400 mt-0.5">Fill in the details to register a new administrator</p>
        </div>
    </div>
        ...
        <form action="{{ route('admins.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid sm:grid-cols-2 grid-cols-1 gap-4 mb-4">
                <div>
                    ...
                    <input class="form-input w-full" name="email" type="email" value="{{ old('email') }}" required placeholder="admin@school.com">
                </div>
                <div>
            ...
            <div class="grid sm:grid-cols-2 grid-cols-1 gap-4 mb-6">
                <div>
                    ...
                    <input class="form-input w-full" name="phone" type="text" value="{{ old('phone') }}" placeholder="+1 555 000 0000">
                </div>
                <div>
            ...
            <div class="grid sm:grid-cols-2 grid-cols-1 gap-4 mb-4">
                <div>
                    ...
                    <input class="form-input w-full" name="employee_id" type="text" value="{{ old('employee_id') }}" required placeholder="e.g. ADM-001">
                </div>
                <div>
                    <label class="block text-sm font-medium text-default-700 mb-1.5">Department</label>
                    <input class="form-input w-full" name="department" type="text" value="{{ old('department') }}" placeholder="e.g. Administration">
                </div>
            </div>
            <div class="mb-6">
                <label class="block text-sm font-medium text-default-700 mb-1.5">Hire Date</label>
                <input class="form-input w-full max-w-xs" name="hire_date" type="date" value="{{ old('hire_date') }}">
            </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-default-100">
                <button type="submit" class="btn bg-primary text-white gap-1.5">
                    <i class="ti ti-device-floppy text-base"></i> Save Admin
                </button>
                <a href="{{ route('admins.index') }}" class="btn bg-default-150 text-default-600">Cancel</a>
            </div>
        </form>
    </div>
</div>
